...
QUOTE_APIS = [
    ...
    "https://quote-api-v2.vercel.app/generate",
    "https://telegram-quote.vercel.app/generate",
]
...
async def get_message_sender_name(ctx: Message):
    if ctx.forward_date:
        if ctx.forward_sender_name:
            ...
            return (
                f"{ctx.forward_from.first_name} {ctx.forward_from.last_name}".strip()
    ...
    if ctx.from_user:
        return (
            f"{ctx.from_user.first_name} {ctx.from_user.last_name}".strip()
            if ctx.from_user.last_name
            else ctx.from_user.first_name
        )
...
async def get_chat_type(ctx: Message):
    if ctx.chat and ctx.chat.type:
        return ctx.chat.type.name.lower()
    return "private"
...
async def generate_quote_with_api(api_url: str, payload: dict) -> bytes:
    ...
    except QuotlyException:
        raise
    except Exception as e:
        raise QuotlyException(f"{api_url}: {type(e).__name__}: {str(e)}")
    ...
    for message in messages:
        ...
        message_dict = {
            ...
            "from": {
                ...
                "type": await get_chat_type(message),
...
async def send_quote_sticker(ctx: Message, image_bytes: bytes, reply_to_id: int = None):
...
async def quotly_handler(client: Client, ctx: Message):
    ...
    try:
        ...
        if count == 1:
            ...
            await send_quote_sticker(ctx, quote_image, reply_to_id=ctx.reply_to_message.id)
        else:
            # Try batch first (all messages in one sticker)
            batch_ok = False
            try:
                quote_image = await pyrogram_to_quotly(messages, is_reply=is_reply)
                await send_quote_sticker(ctx, quote_image, reply_to_id=ctx.reply_to_message.id)
            ...
            # Fall back to one sticker per message
            if not batch_ok:
                sent = 0
                for msg in messages:
                    try:
                        quote_image = await pyrogram_to_quotly([msg], is_reply=False)
                        await send_quote_sticker(ctx, quote_image)
                        sent += 1
                    except QuotlyException:
                        continue

                if sent == 0:
                    raise QuotlyException("All APIs failed for every message in the batch.")
